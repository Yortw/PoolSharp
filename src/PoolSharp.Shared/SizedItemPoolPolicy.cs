...
{
	/// <summary>
	/// Provides configuration controlling how an object pool for a sized item works.
	/// </summary>
	...
	public class SizedItemPoolPolicy<T>
	{
		/// <summary>
		/// A function that returns a new item for the pool. Used when the pool is empty and a new item is requested.
		/// </summary>
		/// <remarks>
		/// <para>Should return a new, clean item, ready for use by the caller. The first argument is a reference to the pool that was asked for the object, useful if you're creating <see cref="PooledObject{T}"/> instances.
		/// The second argument is the size of object required.</para>
		...
		public Func<IPool<T>, int, T> Factory { get; set; }
		/// <summary>
